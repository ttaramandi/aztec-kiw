  ...
  public:
    ...
    bool pairing_check(const GroupElement& p0, const GroupElement& p1)
    {
        ...
        Curve::TargetField result =
            pairing::reduced_ate_pairing_batch_precomputed(pairing_points, srs->get_precomputed_g2_lines(), 2);

        return (result == Curve::TargetField::one());
    }

    std::shared_ptr<srs::factories::VerifierCrs<Curve>> srs;
};
...
template <> class VerifierCommitmentKey<curve::Grumpkin> {
  ...
  public:
     ...
     */
    VerifierCommitmentKey(size_t num_points, std::shared_ptr<srs::factories::CrsFactory<Curve>> crs_factory)
