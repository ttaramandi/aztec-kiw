 ...
 */
template <HasMsgPack T> void msgpack_apply(const auto& func, auto&... args)
{
    std::apply(func, msgpack_drop_keys(std::tie(args...)));
}
/**
 ...
 */
template <HasMsgPack T> void msgpack_apply(const T& value, const auto& func)
{
    ...
    const_cast<T&>(value).msgpack([&](auto&... args) { // NOLINT
        std::apply(static_checker, msgpack_drop_keys(std::tie(args...)));
