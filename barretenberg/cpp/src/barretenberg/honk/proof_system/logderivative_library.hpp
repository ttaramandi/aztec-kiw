...
void compute_logderivative_inverse(Polynomials& polynomials, auto& relation_parameters, const size_t circuit_size)
{
    ...
    for (size_t i = 0; i < circuit_size; ++i) {
        ...
        FF denominator = 1;
        constexpr_for<0, READ_TERMS, 1>([&]<size_t read_index> {
        ...
        constexpr_for<0, WRITE_TERMS, 1>([&]<size_t write_index> {
            auto denominator_term =
                                                               ...
                                                               const FF& scaling_factor)
{
    ...
    constexpr_for<0, READ_TERMS, 1>(
        [&]<size_t i>() { lookup_terms[i] = lookup_relation.template compute_read_term<Accumulator, i>(in, params); });
    constexpr_for<0, WRITE_TERMS, 1>([&]<size_t i>() {
        lookup_terms[i + READ_TERMS] = lookup_relation.template compute_write_term<Accumulator, i>(in, params);
    });

    constexpr_for<0, NUM_TOTAL_TERMS, 1>([&]<size_t i>() { denominator_accumulator[i] = lookup_terms[i]; });

    constexpr_for<0, NUM_TOTAL_TERMS - 1, 1>(
    ...
    constexpr_for<0, READ_TERMS, 1>([&]<size_t i>() {
                                                                    ...
                                                                    const FF& scaling_factor)
{
    ...
    constexpr_for<0, NUM_TOTAL_TERMS, 1>([&]<size_t i>() { denominator_accumulator[i] = permutation_terms[i]; });
