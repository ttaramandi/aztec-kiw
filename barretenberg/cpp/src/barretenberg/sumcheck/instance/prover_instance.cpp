...
template <class Flavor> void ProverInstance_<Flavor>::compute_witness(Circuit& circuit)
{
    ...
    proving_key->witnesses.w_l = wire_polynomials[0];
    proving_key->witnesses.w_r = wire_polynomials[1];
    proving_key->witnesses.w_o = wire_polynomials[2];
    proving_key->witnesses.w_4 = wire_polynomials[3];
    ...
    // space
    proving_key->witnesses.sorted_1 = s_1;
    proving_key->witnesses.sorted_2 = s_2;
    proving_key->witnesses.sorted_3 = s_3;
    proving_key->witnesses.sorted_4 = s_4;
...
template <class Flavor> void ProverInstance_<Flavor>::construct_ecc_op_wire_polynomials(auto& wire_polynomials)
{
    ...
    proving_key->witnesses.ecc_op_wire_1 = op_wire_polynomials[0];
    proving_key->witnesses.ecc_op_wire_2 = op_wire_polynomials[1];
    proving_key->witnesses.ecc_op_wire_3 = op_wire_polynomials[2];
    proving_key->witnesses.ecc_op_wire_4 = op_wire_polynomials[3];
}
    ...
    requires IsGoblinFlavor<Flavor>
{
    ...
    proving_key->witnesses.calldata = public_calldata;
    proving_key->witnesses.calldata_read_counts = calldata_read_counts;
}
...
std::shared_ptr<typename Flavor::ProvingKey> ProverInstance_<Flavor>::compute_proving_key(Circuit& circuit)
{
    ...
    proving_key->precomputed.table_1 = poly_q_table_column_1;
    proving_key->precomputed.table_2 = poly_q_table_column_2;
    proving_key->precomputed.table_3 = poly_q_table_column_3;
    proving_key->precomputed.table_4 = poly_q_table_column_4;
...
template <class Flavor> void ProverInstance_<Flavor>::initialise_prover_polynomials()
{
    ...
    prover_polynomials.table_4_shift = proving_key->precomputed.table_4.shifted();
    prover_polynomials.w_l_shift = proving_key->witnesses.w_l.shifted();
    prover_polynomials.w_r_shift = proving_key->witnesses.w_r.shifted();
    ...
    for (size_t i = 0; i < proving_key->precomputed.num_public_inputs; ++i) {
...
template <class Flavor> void ProverInstance_<Flavor>::compute_sorted_accumulator_polynomials(FF eta)
{
    ...
    prover_polynomials.sorted_accum = proving_key->witnesses.sorted_accum;
    prover_polynomials.sorted_accum_shift = proving_key->witnesses.sorted_accum.shifted();

    // Finalize fourth wire polynomial by adding lookup memory records
    add_plookup_memory_records_to_wire_4(eta);
    prover_polynomials.w_4 = proving_key->witnesses.w_4;
    prover_polynomials.w_4_shift = proving_key->witnesses.w_4.shifted();
}
...
{
    const size_t circuit_size = proving_key->get_circuit_size();
    ...
    proving_key->witnesses.sorted_accum = sorted_list_accumulator;
}
...
template <class Flavor> void ProverInstance_<Flavor>::compute_grand_product_polynomials(FF beta, FF gamma)
{
    auto public_input_delta = compute_public_input_delta<Flavor>(
        public_inputs, beta, gamma, proving_key->get_circuit_size(), pub_inputs_offset);

    relation_parameters.beta = beta;
    relation_parameters.gamma = gamma;
    relation_parameters.public_input_delta = public_input_delta;

    auto lookup_grand_product_delta = compute_lookup_grand_product_delta(beta, gamma, proving_key->get_circuit_size());
...
std::shared_ptr<typename Flavor::VerificationKey> ProverInstance_<Flavor>::compute_verification_key()
{
    ...
    verification_key->q_m = commitment_key->commit(proving_key->precomputed.q_m);
    verification_key->q_l = commitment_key->commit(proving_key->precomputed.q_l);
    verification_key->q_r = commitment_key->commit(proving_key->precomputed.q_r);
    verification_key->q_o = commitment_key->commit(proving_key->precomputed.q_o);
    verification_key->q_c = commitment_key->commit(proving_key->precomputed.q_c);
    verification_key->sigma_1 = commitment_key->commit(proving_key->precomputed.sigma_1);
    verification_key->sigma_2 = commitment_key->commit(proving_key->precomputed.sigma_2);
    verification_key->sigma_3 = commitment_key->commit(proving_key->precomputed.sigma_3);
    verification_key->id_1 = commitment_key->commit(proving_key->precomputed.id_1);
    verification_key->id_2 = commitment_key->commit(proving_key->precomputed.id_2);
    verification_key->id_3 = commitment_key->commit(proving_key->precomputed.id_3);
    ...
    verification_key->q_4 = commitment_key->commit(proving_key->precomputed.q_4);
    verification_key->q_arith = commitment_key->commit(proving_key->precomputed.q_arith);
    verification_key->q_sort = commitment_key->commit(proving_key->precomputed.q_sort);
    verification_key->q_elliptic = commitment_key->commit(proving_key->precomputed.q_elliptic);
    verification_key->q_aux = commitment_key->commit(proving_key->precomputed.q_aux);
    verification_key->q_lookup = commitment_key->commit(proving_key->precomputed.q_lookup);
    verification_key->sigma_4 = commitment_key->commit(proving_key->precomputed.sigma_4);
    verification_key->id_4 = commitment_key->commit(proving_key->precomputed.id_4);
    verification_key->table_1 = commitment_key->commit(proving_key->precomputed.table_1);
    verification_key->table_2 = commitment_key->commit(proving_key->precomputed.table_2);
    verification_key->table_3 = commitment_key->commit(proving_key->precomputed.table_3);
    verification_key->table_4 = commitment_key->commit(proving_key->precomputed.table_4);
    ...
    if constexpr (IsGoblinFlavor<Flavor>) {
        verification_key->lagrange_ecc_op = commitment_key->commit(proving_key->precomputed.lagrange_ecc_op);
        verification_key->q_busread = commitment_key->commit(proving_key->precomputed.q_busread);
    }
