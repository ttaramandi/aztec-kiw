...
{
    stdlib::field_t<Builder> left(stdlib::witness_t(&builder, barretenberg::fr::random_element()));
    stdlib::field_t<Builder> out(stdlib::witness_t(&builder, barretenberg::fr::random_element()));
...
void generate_test_pedersen_hash_buffer_circuit(Builder& builder, size_t num_repetitions)
{
    ...
    for (size_t i = 0; i < num_repetitions; ++i) {
        stdlib::byte_array<Builder> tmp(stdlib::witness_t(&builder, barretenberg::fr::random_element()));
        input.write(tmp);
    }
