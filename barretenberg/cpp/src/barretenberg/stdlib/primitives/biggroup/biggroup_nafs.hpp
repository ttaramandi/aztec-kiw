...
typename element<C, Fq, Fr, G>::secp256k1_wnaf_pair element<C, Fq, Fr, G>::compute_secp256k1_endo_wnaf(const Fr& scalar)
{
    ...
    const auto compute_single_wnaf = [ctx](const secp256k1::fr& k,
        ...
        if (is_lo) {
            wnaf::fixed_wnaf<num_bits - lo_stagger, 1, wnaf_size>(
                &k_u256.data[0], &wnaf_values[0], skew_without_stagger, 0);
        } else {
            wnaf::fixed_wnaf<num_bits - hi_stagger, 1, wnaf_size>(
                &k_u256.data[0], &wnaf_values[0], skew_without_stagger, 0);
        }
        ...
        const auto reconstruct_bigfield_from_wnaf = [ctx](const std::vector<field_t<C>>& wnaf,
            ...
            sum = sum * field_t<C>(fr(1ULL << stagger));
    ...
    Fr minus_lambda(bb::fr(minus_lambda_val.slice(0, 136)), fr(minus_lambda_val.slice(136, 256)), false);
...
std::vector<field_t<C>> element<C, Fq, Fr, G>::compute_wnaf(const Fr& scalar)
{
    ...
    uint64_t wnaf_values[num_rounds] = { 0 };
    bool skew = false;
    wnaf::fixed_wnaf<num_bits, 1, WNAF_SIZE>(&scalar_multiplier.data[0], &wnaf_values[0], skew, 0);
    ...
    if constexpr (!Fr::is_composite) {
        ...
        field_t<C> lo_offset = (-field_t<C>(fr(negative_lo)))
                                   .madd(wnaf_entries[wnaf_entries.size() - 1], field_t<C>(fr(negative_lo)))
                                   .normalize();
        Fr offset = Fr(lo_offset, field_t<C>(fr(negative_hi)) + wnaf_entries[wnaf_entries.size() - 1], true);
