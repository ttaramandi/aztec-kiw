...
// designed to have efficient g1 operations, a developer might accidentally write inefficient circuits
...
concept IsNotGoblinInefficiencyTrap = !(IsGoblinBuilder<Builder> && std::same_as<NativeGroup, g1>);
...
template <class Builder, class Fq, class Fr, class NativeGroup> class element {
  public:
    ...
    // Our input to `std::enable_if` is a comparison between two types (NativeGroup and g1), which
    ...
    template <typename X = NativeGroup, typename = typename std::enable_if_t<std::is_same<X, g1>::value>>
