...
template <typename Builder> class BitArrayFuzzBase {
  private:
    typedef stdlib::bit_array<Builder> bit_array_t;
    typedef stdlib::byte_array<Builder> byte_array_t;
    template <size_t NumBytes, size_t NumWords>
    static std::vector<uint8_t> to_vector(std::array<stdlib::uint32<Builder>, NumWords>& a32)
    {
    ...
    static std::vector<uint8_t> vector_via_populate_uint32_array(bit_array_t& bit_array, const size_t offset = 0)
    {
        ...
        if (offset > bit_array.size() || (bit_array.size() - offset) % 32 != 0) {
            ...
            std::array<stdlib::uint32<Builder>, NumWords> a32;
        ...
        static std::vector<uint8_t> get_value(bit_array_t& bit_array)
        {
            ...
            case 3:
                ...
                if (bit_array.size() == MAX_ARRAY_SIZE / 32) {
                    ...
                    const auto a32_ = static_cast<std::array<stdlib::uint32<Builder>, MAX_ARRAY_SIZE>>(bit_array);
...
{
    using FuzzerClass = BitArrayFuzzBase<StandardCircuitBuilder>;
