 ...
 *
 * Let r = fr::modulus. For later applications, we want to ensure that the prover must pass the bit
...
template <typename Builder> byte_array<Builder>::operator field_t<Builder>() const
{
    ...
    fr shift(256);
    field_t<Builder> result(context, fr(0));
    for (size_t i = 0; i < values.size(); ++i) {
        field_t<Builder> temp(values[i]);
        fr scaling_factor_value = shift.pow(static_cast<uint64_t>(bytes - 1 - i));
...
template <typename Builder> void byte_array<Builder>::set_bit(size_t index_reversed, bool_t<Builder> const& new_bit)
{
    ...
    field_t<Builder> scaled_new_bit = field_t<Builder>(new_bit) * fr(uint256_t(1) << bit_index);
...
typename byte_array<Builder>::byte_slice byte_array<Builder>::split_byte(const size_t index) const
{
    ...
    field_t<Builder> scaled_high = high * fr(uint256_t(1) << (8ULL - num_high_bits));
    field_t<Builder> scaled_bit = field_t<Builder>(bit) * fr(uint256_t(1) << bit_index);
...
template class byte_array<StandardCircuitBuilder>;
template class byte_array<UltraCircuitBuilder>;
template class byte_array<GoblinUltraCircuitBuilder>;
