...
template <typename Builder> class witness_t {
  public:
    ...
    witness_t(Builder* parent_context, const fr& in)
    {
    ...
    witness_t(Builder* parent_context, const bool in)
    {
        context = parent_context;
        if (in) {
            fr::__copy(fr::one(), witness);
        } else {
            fr::__copy(fr::zero(), witness);
        }
        witness_index = context->add_variable(witness);
    }
    ...
    {
        context = parent_context;
        witness = fr{ static_cast<uint64_t>(in), 0, 0, 0 }.to_montgomery_form();
        witness_index = context->add_variable(witness);
    }

    static witness_t create_constant_witness(Builder* parent_context, const fr& in)
    {
...
template <typename Builder> class public_witness_t : public witness_t<Builder> {
  public:
    ...
    public_witness_t(Builder* parent_context, const fr& in)
    {
        context = parent_context;
        fr::__copy(in, witness);
