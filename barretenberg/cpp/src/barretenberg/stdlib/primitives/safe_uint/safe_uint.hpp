...
namespace bb::stdlib {
{
    ...
    template <typename Builder> class safe_uint_t {
      private:
        ...
        // this constructor is private since we only want the operators to be able to define a positive int without a
        // range check.
      ...
      public:
        ...
        {
            ASSERT((uint512_t)current_max * (uint512_t)to_mul.current_max + (uint512_t)to_add.current_max <=
                       MAX_VALUE &&
