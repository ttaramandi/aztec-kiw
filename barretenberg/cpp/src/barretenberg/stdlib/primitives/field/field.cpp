...
template <typename Builder>
field_t<Builder>::field_t(Builder* parent_context, const fr& value)
    : context(parent_context)
{
    additive_constant = value;
    multiplicative_constant = fr::zero();
    witness_index = IS_CONSTANT;
}
...
template <typename Builder> field_t<Builder>::field_t(const bool_t<Builder>& other)
{
    ...
    if (other.witness_index == IS_CONSTANT) {
        additive_constant = (other.witness_bool ^ other.witness_inverted) ? bb::fr::one() : fr::zero();
        multiplicative_constant = fr::one();
        witness_index = IS_CONSTANT;
    } else {
        witness_index = other.witness_index;
        additive_constant = other.witness_inverted ? bb::fr::one() : fr::zero();
        multiplicative_constant = other.witness_inverted ? bb::fr::neg_one() : fr::one();
    }
}
...
template <typename Builder> field_t<Builder>::operator bool_t<Builder>() const
{
    if (witness_index == IS_CONSTANT) {
        ...
        result.witness_bool = (additive_constant == fr::one());
        result.witness_inverted = false;
        result.witness_index = IS_CONSTANT;
        return result;
    }
    bool add_constant_check = (additive_constant == fr::zero());
    bool mul_constant_check = (multiplicative_constant == fr::one());
    bool inverted_check = (additive_constant == bb::fr::one()) && (multiplicative_constant == fr::neg_one());
    if ((!add_constant_check || !mul_constant_check) && !inverted_check) {
        normalize();
    }

    fr witness = context->get_variable(witness_index);
    ASSERT((witness == bb::fr::zero()) || (witness == fr::one()));
    bool_t<Builder> result(context);
    result.witness_bool = (witness == fr::one());
...
template <typename Builder> field_t<Builder> field_t<Builder>::operator+(const field_t& other) const
{
    ...
    if (witness_index == other.witness_index) {
        ...
        fr T0;
        fr left = ctx->get_variable(witness_index);
        fr right = ctx->get_variable(other.witness_index);
        fr out;
...
template <typename Builder> field_t<Builder> field_t<Builder>::operator*(const field_t& other) const
{
    ...
    if (witness_index == IS_CONSTANT && other.witness_index == IS_CONSTANT) {
        ...
        fr left = context->get_variable(witness_index);
        fr right = context->get_variable(other.witness_index);
        fr out;
...
template <typename Builder> field_t<Builder> field_t<Builder>::divide_no_zero_check(const field_t& other) const
{
    ...
    fr additive_multiplier = fr::one();

    if (witness_index == IS_CONSTANT && other.witness_index == IS_CONSTANT) {
        // both inputs are constant - don't add a gate
        if (!(other.additive_constant == fr::zero())) {
        ...
        if (get_value() == 0) {
            ...
            fr q_m = other.multiplicative_constant;
            fr q_l = other.additive_constant;
            fr q_c = -get_value();
            fr out_value = get_value() / other.get_value();
        ...
        // fr witness_multiplier = other.witness.invert();
        ...
        fr q_l = other.additive_constant;
        fr q_r = fr::zero();
        fr q_o = -multiplicative_constant;
        fr q_c = -additive_constant;
...
template <typename Builder> field_t<Builder> field_t<Builder>::madd(const field_t& to_mul, const field_t& to_add) const
{
    ...
    fr q_m = multiplicative_constant * to_mul.multiplicative_constant;
    fr q_1 = multiplicative_constant * to_mul.additive_constant;
    fr q_2 = to_mul.multiplicative_constant * additive_constant;
    fr q_3 = to_add.multiplicative_constant;
    fr q_c = additive_constant * to_mul.additive_constant + to_add.additive_constant;
    ...
    fr a = witness_index == IS_CONSTANT ? fr(0) : ctx->get_variable(witness_index);
    fr b = to_mul.witness_index == IS_CONSTANT ? fr(0) : ctx->get_variable(to_mul.witness_index);
    fr c = to_add.witness_index == IS_CONSTANT ? fr(0) : ctx->get_variable(to_add.witness_index);

    fr out = a * b * q_m + a * q_1 + b * q_2 + c * q_3 + q_c;
...
template <typename Builder> field_t<Builder> field_t<Builder>::add_two(const field_t& add_a, const field_t& add_b) const
{
    ...
    fr q_1 = multiplicative_constant;
    fr q_2 = add_a.multiplicative_constant;
    fr q_3 = add_b.multiplicative_constant;
    fr q_c = additive_constant + add_a.additive_constant + add_b.additive_constant;

    fr a = witness_index == IS_CONSTANT ? fr(0) : ctx->get_variable(witness_index);
    fr b = add_a.witness_index == IS_CONSTANT ? fr(0) : ctx->get_variable(add_a.witness_index);
    fr c = add_b.witness_index == IS_CONSTANT ? fr(0) : ctx->get_variable(add_b.witness_index);

    fr out = a * q_1 + b * q_2 + c * q_3 + q_c;
...
{
    if (witness_index == IS_CONSTANT ||
        ((multiplicative_constant == bb::fr::one()) && (additive_constant == fr::zero()))) {
        return *this;
    }
    ...
    fr value = context->get_variable(witness_index);
    fr out;
    out = value * multiplicative_constant;
    out += additive_constant;

    result.witness_index = context->add_variable(out);
    result.additive_constant = fr::zero();
    result.multiplicative_constant = fr::one();
...
template <typename Builder> void field_t<Builder>::assert_is_not_zero(std::string const& msg) const
{
    ...
    fr inverse_value = (get_value() == 0) ? 0 : get_value().invert();
...
{
    if (witness_index == IS_CONSTANT) {
        return bool_t(context, (get_value() == fr::zero()));
    }
    ...
    field_t k = normalize();
    bool_t is_zero = witness_t(context, (k.get_value() == fr::zero()));
    field_t k_inverse;
    if (is_zero.get_value()) {
        k_inverse = witness_t(context, fr::one());
    } else {
        fr k_inverse_value = k.get_value().invert();
...
template <typename Builder> fr field_t<Builder>::get_value() const
{
    if (witness_index != IS_CONSTANT) {
...
template <typename Builder> bool_t<Builder> field_t<Builder>::operator==(const field_t& other) const
{
    ...
    fr fa = get_value();
    fr fb = other.get_value();
    fr fd = fa - fb;
    bool is_equal = (fa == fb);
    fr fc = is_equal ? fr::one() : fd.invert();
        ...
        if (is_constant()) {
            ...
            if constexpr (HasPlookup<Builder>) {
                context->decompose_into_default_range(
                    normalize().get_witness_index(), num_bits, UltraCircuitBuilder::DEFAULT_PLOOKUP_RANGE_BITNUM, msg);
            } else {
...
void field_t<Builder>::evaluate_linear_identity(const field_t& a, const field_t& b, const field_t& c, const field_t& d)
{
    ...
    fr q_1 = a.multiplicative_constant;
    fr q_2 = b.multiplicative_constant;
    fr q_3 = c.multiplicative_constant;
    fr q_4 = d.multiplicative_constant;
    fr q_c = a.additive_constant + b.additive_constant + c.additive_constant + d.additive_constant;
                                                    ...
                                                    const field_t& d)
{
    ...
    // validate that a * b + c + d = 0
    fr q_m = a.multiplicative_constant * b.multiplicative_constant;
    fr q_1 = a.multiplicative_constant * b.additive_constant;
    fr q_2 = b.multiplicative_constant * a.additive_constant;
    fr q_3 = c.multiplicative_constant;
    fr q_4 = d.multiplicative_constant;
    fr q_c = a.additive_constant * b.additive_constant + c.additive_constant + d.additive_constant;
...
template <typename Builder> field_t<Builder> field_t<Builder>::accumulate(const std::vector<field_t>& input)
{
    ...
    if constexpr (HasPlookup<Builder>) {
        ...
        for (size_t i = 0; i < num_gates; ++i) {
            ...
            fr new_total = accumulating_total.get_value() - accumulator[3 * i].get_value() -
    ...
    const size_t num_bits, const std::function<witness_t<Builder>(Builder*, uint64_t, uint256_t)> get_bit) const
{
    ...
    for (size_t i = 0; i < num_bits; ++i) {
        ...
        fr scaling_factor_value = fr(2).pow(static_cast<uint64_t>(num_bits - 1 - i));
...
template class field_t<StandardCircuitBuilder>;
template class field_t<UltraCircuitBuilder>;
template class field_t<GoblinUltraCircuitBuilder>;
