...
template <typename Builder, typename T> class bigfield {
  ...
  public:
    ...
    static bigfield from_witness(Builder* ctx, const field<T>& input)
    {
        uint256_t input_u256(input);
        field_t<Builder> low(witness_t<Builder>(ctx, fr(input_u256.slice(0, NUM_LIMB_BITS * 2))));
        field_t<Builder> hi(witness_t<Builder>(ctx, fr(input_u256.slice(NUM_LIMB_BITS * 2, NUM_LIMB_BITS * 4))));
    ...
    static constexpr Basis prime_basis{ uint512_t(bb::fr::modulus), fr::modulus.get_msb() + 1 };
    ...
    static constexpr bb::fr shift_1 = fr(uint256_t(1) << NUM_LIMB_BITS);
    static constexpr bb::fr shift_2 = fr(uint256_t(1) << (NUM_LIMB_BITS * 2));
    static constexpr bb::fr shift_3 = fr(uint256_t(1) << (NUM_LIMB_BITS * 3));
    static constexpr bb::fr shift_right_1 = fr(1) / shift_1;
    static constexpr bb::fr shift_right_2 = fr(1) / shift_2;
    static constexpr bb::fr negative_prime_modulus_mod_binary_basis = -fr(uint256_t(modulus_u512));
    ...
    static constexpr fr neg_modulus_limbs[4]{
    ...
    static constexpr bigfield unreduced_zero()
    {
        ...
        bigfield result(nullptr, uint256_t(0));
        result.binary_basis_limbs[0] = Limb(fr(multiple_of_modulus.slice(0, NUM_LIMB_BITS).lo));
        result.binary_basis_limbs[1] = Limb(fr(multiple_of_modulus.slice(NUM_LIMB_BITS, 2 * NUM_LIMB_BITS).lo));
        result.binary_basis_limbs[2] = Limb(fr(multiple_of_modulus.slice(2 * NUM_LIMB_BITS, 3 * NUM_LIMB_BITS).lo));
        result.binary_basis_limbs[3] = Limb(fr(multiple_of_modulus.slice(3 * NUM_LIMB_BITS, msb + 1).lo));
    ...
    static constexpr uint64_t MAX_UNREDUCED_LIMB_SIZE = (fr::modulus.get_msb() + 1) / 2 - MAX_ADDITION_LOG;
