...
bigfielddyn<Builder>::bigfielddyn(Builder* parent_context)
...
template <typename Builder>
bigfielddyn<Builder>::bigfielddyn(Builder* parent_context, const uint256_t& value)
    : context(parent_context)
...
template <typename Builder>
bigfielddyn<Builder>::bigfielddyn(bigfielddyn&& other)
    : context(other.context)
    , binary_basis_limbs{ other.binary_basis_limbs[0],
                          other.binary_basis_limbs[1],
                          other.binary_basis_limbs[2],
 ...
 */
template <typename Builder>
bigfielddyn<Builder> bigfielddyn<Builder>::create_from_u512_as_witness(
    Builder* ctx, const uint512_t& value, uint512_t modulus, const bool can_overflow, const size_t maximum_bitlength)
{
    ASSERT((can_overflow == true && maximum_bitlength == 0) ||
           (can_overflow == false && (maximum_bitlength == 0 || maximum_bitlength > (3 * NUM_LIMB_BITS))));
...
template <typename Builder> bigfielddyn<Builder>::bigfielddyn(const byte_array<Builder>& bytes, uint512_t modulus)
{
    ...
    const auto res = bigfielddyn(limb0, limb1, limb2, limb3, true, modulus);
...
template <typename Builder> bigfielddyn<Builder> bigfielddyn<Builder>::operator+(const bigfielddyn& other) const
{
    ...
    if (is_constant() && other.is_constant()) {
        return bigfielddyn(ctx, uint256_t((get_value() + other.get_value()) % modulus_u512));
    }
    bigfielddyn result(ctx);
...
template <typename Builder> void bigfielddyn<Builder>::assert_is_not_equal(const bigfielddyn& other) const
{
    ...
    auto diff = base_diff;
    field_t<Builder> prime_basis(get_context(), modulus_u512);
...
std::pair<uint512_t, uint512_t> bigfielddyn<Builder>::compute_quotient_remainder_values(
    const bigfielddyn& a, const bigfielddyn& b, const std::vector<bigfielddyn>& to_add)
{
    uint512_t add_values(0);
    for (const auto& add_element : to_add) {
        add_element.reduction_check();
...
template <typename Builder>
std::pair<bool, size_t> bigfielddyn<Builder>::get_quotient_reduction_info(const std::vector<uint512_t>& as_max,
