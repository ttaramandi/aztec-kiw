...
template <typename Builder> class bigfielddyn {
  ...
  public:
    typedef bb::field<Builder> native;
    ...
    bigfielddyn(const field_t<Builder>& low_bits,
                const field_t<Builder>& high_bits,
                const bool can_overflow = false,
                const size_t maximum_bitlength = 0);
    bigfielddyn(Builder* parent_context = nullptr);
    bigfielddyn(Builder* parent_context, const uint256_t& value);
    ...
    static bigfielddyn from_witness(Builder* ctx, const bb::field<Builder>& input, uint512_t modulus)
    {
        ...
        return bigfield(low, hi, modulus);
    }
    ...
    Basis target_basis{ modulus_u512, modulus_u512.get_msb() + 1 };
    ...
    bigfielddyn operator/(const bigfielddyn& other) const;
    bigfielddyn operator-() const { return bigfield(get_context(), uint256_t(0)) - *this; }
     ...
     */
    size_t get_quotient_max_bits(const std::vector<uint1024_t>& remainders_max)
    {
  ...
  private:
    ...
    void unsafe_evaluate_multiply_add(const bigfielddyn& left,
                                      ...
                                      const std::vector<bigfielddyn>& remainders);
