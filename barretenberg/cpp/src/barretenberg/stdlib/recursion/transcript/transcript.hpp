...
template <typename Builder> class Transcript {
  public:
    ...
    using fq_pt = bigfield<Builder, Bn254FqParams>;
    using group_pt = element<Builder, fq_pt, field_pt, g1>;
    ...
    void add_group_element(const std::string& element_name, const group_pt& element)
    {
        ...
        g1::affine_element converted{ bb::fq(x), fq(y) };
    ...
    field_pt get_field_element(const std::string& element_name) const
    {
        ...
        fr value = fr::serialize_from_buffer(&(transcript_base.get_element(element_name))[0]);
    ...
    std::vector<field_pt> get_field_element_vector(const std::string& element_name) const
    {
        ...
        std::vector<fr> values = many_from_buffer<fr>(transcript_base.get_element(element_name));
    ...
    group_pt get_circuit_group_element(const std::string& element_name) const
    {
        ...
        g1::affine_element value =
            g1::affine_element::serialize_from_buffer(&(transcript_base.get_element(element_name))[0]);
    ...
    static fq_pt convert_fq(Builder* ctx, const fq& input) { return fq_pt::from_witness(ctx, input); };

    static group_pt convert_g1(Builder* ctx, const g1::affine_element& input)
    {
