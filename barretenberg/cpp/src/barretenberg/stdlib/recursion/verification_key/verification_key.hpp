...
template <class Builder, size_t bits_per_element = 248> struct PedersenPreimageBuilder {
    ...
    field_pt hash()
    {
        ...
        static_assert(bits_per_element < uint256_t(fr::modulus).get_msb());
    ...
    void slice_element(const field_pt& element, const size_t num_bits)
    {
        ...
        if (hi_bits >= num_bits) {
            ...
            field_pt buffer_shift = field_pt(context, fr(uint256_t(1) << ((uint64_t)leftovers)));
        ...
        field_pt lo = witness_t(context, fr(element_u256.slice(0, lo_bits)));
        field_pt hi = witness_t(context, fr(element_u256.slice(lo_bits, 256)));
        lo.create_range_constraint(lo_bits);
        hi.create_range_constraint(hi_bits);
        field_pt shift(context, fr(uint256_t(1ULL) << static_cast<uint64_t>(lo_bits)));
        ...
        if (current_bit_counter == 0) {
            ...
            field_t lo_shift(context,
                             fr(uint256_t(1ULL) << ((bits_per_element - static_cast<uint64_t>(current_bit_counter)))));
            work_element = std::vector<field_pt>();
...
template <typename Builder> struct evaluation_domain {
    ...
    static evaluation_domain from_witness(Builder* ctx, const evaluation_domain& input)
    {
        evaluation_domain domain;
    ...
    static evaluation_domain from_constants(Builder* ctx, const evaluation_domain& input)
    {
        evaluation_domain domain;
...
template <typename Curve> struct verification_key {
                                                          ...
                                                          const std::shared_ptr<plonk::verification_key>& input_key)
    {
        ...
        // Circuit types:
        key->n = witness_t<Builder>(ctx, fr(input_key->circuit_size));
    ...
    void validate_key_is_in_set(const std::vector<std::shared_ptr<plonk::verification_key>>& keys_in_set)
    {
        ...
        if constexpr (HasPlookup<Builder>) {
            ...
            for (size_t i = 0; i < keys_in_set.size(); ++i) {
                fr compressed = hash_native(keys_in_set[i]);
  ...
  public:
    ...
    static fr hash_native(const std::shared_ptr<plonk::verification_key>& key, const size_t hash_index = 0)
    {
    ...
    std::shared_ptr<srs::factories::VerifierCrs<curve::BN254>> reference_string;
