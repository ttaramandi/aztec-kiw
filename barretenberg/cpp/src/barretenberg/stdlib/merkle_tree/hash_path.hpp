...
inline std::ostream& operator<<(std::ostream& os, stdlib::merkle_tree::hash_path<Ctx> const& path)
{
    os << "[\n";
...
inline std::ostream& operator<<(std::ostream& os, stdlib::merkle_tree::fr_hash_path const& path)
{
    os << "[\n";
    for (size_t i = 0; i < path.size(); ++i) {
        os << "  (" << i << ": " << path[i].first << ", " << path[i].second << ")\n";
