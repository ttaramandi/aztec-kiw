...
inline bb::fr hash_pair_native(bb::fr const& lhs, fr const& rhs)
{
    return crypto::pedersen_hash::hash({ lhs, rhs }); // uses lookup tables
}

inline bb::fr hash_native(std::vector<fr> const& inputs)
{
 ...
 */
inline bb::fr compute_tree_root_native(std::vector<fr> const& input)
{
    ...
    auto layer = input;
    while (layer.size() > 1) {
        std::vector<fr> next_layer(layer.size() / 2);
...
// TODO write test
inline std::vector<bb::fr> compute_tree_native(std::vector<fr> const& input)
{
    // Check if the input vector size is a power of 2.
    ASSERT(input.size() > 0);
    ASSERT(numeric::is_power_of_two(input.size()));
    auto layer = input;
    std::vector<fr> tree(input);
    while (layer.size() > 1) {
        std::vector<fr> next_layer(layer.size() / 2);
        for (size_t i = 0; i < next_layer.size(); ++i) {
            next_layer[i] = crypto::pedersen_hash::hash({ layer[i * 2], layer[i * 2 + 1] });
