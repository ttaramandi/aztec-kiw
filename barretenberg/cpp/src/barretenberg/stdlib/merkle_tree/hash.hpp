...
inline fr hash_pair_native(fr const& lhs, fr const& rhs)
{
    return crypto::pedersen_hash::hash({ lhs, rhs }); // uses lookup tables
}

inline fr hash_native(std::vector<fr> const& inputs)
{
 ...
 */
inline fr compute_tree_root_native(std::vector<fr> const& input)
{
...
// TODO write test
inline std::vector<fr> compute_tree_native(std::vector<fr> const& input)
{
    // Check if the input vector size is a power of 2.
    ASSERT(input.size() > 0);
    ASSERT(numeric::is_power_of_two(input.size()));
