...
template <typename Flavor> bool MergeVerifier_<Flavor>::verify_proof(const plonk::proof& proof)
{
    ...
    for (size_t idx = 0; idx < Flavor::NUM_WIRES; ++idx) {
        ...
        opening_claims.emplace_back(bb::OpeningClaim<Curve>{ { kappa, T_prev_evals[idx] }, C_T_prev[idx] });
    }
    for (size_t idx = 0; idx < Flavor::NUM_WIRES; ++idx) {
        t_shift_evals[idx] = transcript->template receive_from_prover<FF>("t_shift_eval_" + std::to_string(idx + 1));
        opening_claims.emplace_back(bb::OpeningClaim<Curve>{ { kappa, t_shift_evals[idx] }, C_t_shift[idx] });
    }
    for (size_t idx = 0; idx < Flavor::NUM_WIRES; ++idx) {
        T_current_evals[idx] =
            transcript->template receive_from_prover<FF>("T_current_eval_" + std::to_string(idx + 1));
        opening_claims.emplace_back(bb::OpeningClaim<Curve>{ { kappa, T_current_evals[idx] }, C_T_current[idx] });
    }
