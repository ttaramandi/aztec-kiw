...
class AcirComposer {
  ...
  public:
    ...
    std::shared_ptr<plonk::proving_key> init_proving_key();

    std::vector<uint8_t> create_proof(bool is_recursive);

    void load_verification_key(plonk::verification_key_data&& data);

    std::shared_ptr<plonk::verification_key> init_verification_key();
    ...
    std::vector<fr> serialize_proof_into_fields(std::vector<uint8_t> const& proof, size_t num_inner_public_inputs);

    std::vector<fr> serialize_verification_key_into_fields();

  private:
    acir_format::Builder builder_;
    size_t size_hint_;
    std::shared_ptr<plonk::proving_key> proving_key_;
    std::shared_ptr<plonk::verification_key> verification_key_;
    bool verbose_ = true;
