 ...
 *
 * */
inline void compute_standard_plonk_lagrange_polynomial(polynomial& output,
                                                       ...
                                                       const evaluation_domain& small_domain)
{
    ...
    if (permutation[i].is_public_input) {
        ...
        output[i] *= fr::external_coset_generator();
    } else if (permutation[i].is_tag) {
        output[i] *= fr::tag_coset_generator();
    } else {
        {
    ...
    plonk::proving_key* key)
{
    for (size_t i = 0; i < program_width; i++) {
        ...
        polynomial polynomial_lagrange(key->circuit_size);
...
void compute_monomial_and_coset_fft_polynomials_from_lagrange(std::string label, plonk::proving_key* key)
{
    for (size_t i = 0; i < program_width; ++i) {
        ...
        polynomial sigma_polynomial(key->circuit_size);
        polynomial_arithmetic::ifft((fr*)&sigma_polynomial_lagrange[0], &sigma_polynomial[0], key->small_domain);

        // Compute permutation polynomial coset FFT form
        polynomial sigma_fft(sigma_polynomial, key->large_domain.size);
