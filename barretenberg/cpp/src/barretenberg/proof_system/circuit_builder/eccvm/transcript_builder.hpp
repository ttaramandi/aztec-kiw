...
template <typename Flavor> class ECCVMTranscriptBuilder {
  public:
    ...
    static std::vector<TranscriptState> compute_transcript_state(
        const std::vector<eccvm::VMOperation<CycleGroup>>& vm_operations, const uint32_t total_number_of_muls)
    {
        ...
        for (size_t i = 0; i < vm_operations.size(); ++i) {
            TranscriptState row;
            const eccvm::VMOperation<CycleGroup>& entry = vm_operations[i];
