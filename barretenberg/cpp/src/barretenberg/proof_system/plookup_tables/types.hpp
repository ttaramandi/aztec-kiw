...
struct MultiTable {
    ...
    std::vector<fr> column_1_step_sizes;
    std::vector<fr> column_2_step_sizes;
    std::vector<fr> column_3_step_sizes;
    typedef std::array<fr, 2> table_out;
  ...
  private:
    void init_step_sizes()
    {
        ...
        column_1_step_sizes.emplace_back(fr(1));
        column_2_step_sizes.emplace_back(fr(1));
        column_3_step_sizes.emplace_back(fr(1));

        std::vector<fr> coefficient_inverses(column_1_coefficients.begin(), column_1_coefficients.end());
  ...
  public:
    MultiTable(const fr& col_1_repeated_coeff,
               const fr& col_2_repeated_coeff,
               const fr& col_3_repeated_coeff,
    ...
    MultiTable(const std::vector<fr>& col_1_coeffs,
               const std::vector<fr>& col_2_coeffs,
               const std::vector<fr>& col_3_coeffs)
...
//     struct KeyEntry {
//         uint256_t key;
//         std::array<fr, 2> value{ bb::fr(0), fr(0) };
...
//     fr column_1_step_size = fr(0);
//     fr column_2_step_size = fr(0);
//     fr column_3_step_size = fr(0);
