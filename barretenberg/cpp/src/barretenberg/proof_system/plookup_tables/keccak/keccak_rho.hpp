...
template <size_t TABLE_BITS = 0, size_t LANE_INDEX = 0> class Rho {
  ...
  public:
     ...
     * @return std::array<fr, 2> table output (normalized input and normalized input / 11^TABLE_BITS - 1)
     */
    static std::array<fr, 2> get_rho_renormalization_values(const std::array<uint64_t, 2> key)
    {
        ...
        return { bb::fr(accumulator), fr(accumulator / divisor) };
    }
    ...
    static BasicTable generate_rho_renormalization_table(BasicTableId id, const size_t table_index)
    {
        ...
        table.column_1_step_size = fr(step_size);
        table.column_2_step_size = fr(step_size);
    ...
    static MultiTable get_rho_output_table(const MultiTableId id = KECCAK_NORMALIZE_AND_ROTATE)
    {
        ...
        constexpr_for<0, num_right_tables, 1>([&]<size_t i> {
        ...
        constexpr_for<0, num_left_tables, 1>([&]<size_t i> {
            constexpr size_t num_bits_processed = (i * MAXIMUM_MULTITABLE_BITS);
