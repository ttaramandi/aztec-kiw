...
class KeccakInput {
  ...
  public:
     ...
     * @return std::array<fr, 2> table output
     */
    static std::array<fr, 2> get_keccak_input_values(const std::array<uint64_t, 2> key)
    {
    ...
    static BasicTable generate_keccak_input_table(BasicTableId id, const size_t table_index)
    {
        ...
        for (uint64_t i = 0; i < table.size; ++i) {
            ...
            table.column_3.emplace_back(fr(source >> msb_shift));
        }
        ...
        table.column_1_step_size = fr((1 << TABLE_BITS));
