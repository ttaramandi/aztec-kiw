...
inline std::array<fr, 2> get_sparse_table_with_rotation_values(const std::array<uint64_t, 2> key)
{
...
inline BasicTable generate_sparse_table_with_rotation(BasicTableId id, const size_t table_index)
{
    ...
    for (uint64_t i = 0; i < table.size; ++i) {
        ...
        table.column_1.emplace_back(fr(source));
        table.column_2.emplace_back(fr(target));

        if constexpr (num_rotated_bits > 0) {
            const auto rotated =
                numeric::map_into_sparse_form<base>(numeric::rotate32((uint32_t)source, num_rotated_bits));
            table.column_3.emplace_back(fr(rotated));
        } else {
            table.column_3.emplace_back(fr(target));
        }
    }
    ...
    table.column_1_step_size = fr((1 << 11));
    table.column_2_step_size = fr(sparse_step_size);
    table.column_3_step_size = fr(sparse_step_size);

    return table;
}

template <size_t base, const uint64_t* base_table>
inline std::array<fr, 2> get_sparse_normalization_values(const std::array<uint64_t, 2> key)
{
...
inline BasicTable generate_sparse_normalization_table(BasicTableId id, const size_t table_index)
{
    ...
    table.column_1_step_size = fr(table.size);
    table.column_2_step_size = fr(((uint64_t)1 << num_bits));
