...
class StandardComposer {
  public:
    ...
    std::shared_ptr<srs::factories::CrsFactory<curve::BN254>> crs_factory_;

    bool computed_witness = false;

    StandardComposer() { crs_factory_ = srs::get_crs_factory(); }
    StandardComposer(std::shared_ptr<srs::factories::CrsFactory<curve::BN254>> crs_factory)
        : crs_factory_(std::move(crs_factory))
    {}

    StandardComposer(std::unique_ptr<srs::factories::CrsFactory<curve::BN254>>&& crs_factory)
        : crs_factory_(std::move(crs_factory))
    {}
