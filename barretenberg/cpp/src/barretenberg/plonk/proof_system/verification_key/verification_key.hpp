...
struct verification_key_data {
    ...
    std::map<std::string, g1::affine_element> commitments;
    ...
    [[nodiscard]] fr hash_native(size_t hash_index = 0) const;
};
...
struct verification_key {
    ...
    fr z_pow_n; // ʓ^n (ʓ being the 'evaluation challenge')
    ...
    void msgpack_unpack(auto obj)
    {
        ...
        *this = verification_key{ std::move(data), srs::get_crs_factory()->get_verifier_crs() };
    }
    // Alias verification_key as verification_key_data in the schema
    void msgpack_schema(auto& packer) const { packer.pack_schema(plonk::verification_key_data{}); }
};
