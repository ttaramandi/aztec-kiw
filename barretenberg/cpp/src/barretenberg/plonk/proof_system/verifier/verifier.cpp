...
template <typename program_settings> bool VerifierBase<program_settings>::verify_proof(const plonk::proof& proof)
{
    ...
    // by each widget.
    const auto lagrange_evals = polynomial_arithmetic::get_lagrange_evaluations(zeta, key->domain);
    ...
    scalar_multiplication::generate_pippenger_point_table<curve::BN254>(&elements[0], &elements[0], num_elements);
    scalar_multiplication::pippenger_runtime_state<curve::BN254> state(num_elements);

    g1::element P[2];

    P[0] = scalar_multiplication::pippenger<curve::BN254>(&scalars[0], &elements[0], num_elements, state);
    ...
    fq12 result =
        pairing::reduced_ate_pairing_batch_precomputed(P_affine, key->reference_string->get_precomputed_g2_lines(), 2);
