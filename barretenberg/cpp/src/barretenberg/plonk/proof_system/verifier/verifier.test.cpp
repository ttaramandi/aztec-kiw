...
plonk::Verifier generate_verifier(std::shared_ptr<proving_key> circuit_proving_key)
{
    ...
    auto crs = std::make_shared<srs::factories::FileVerifierCrs<curve::BN254>>("../srs_db/ignition");
...
plonk::Prover generate_test_data(const size_t n)
{
    ...
    auto crs = std::make_shared<srs::factories::FileProverCrs<curve::BN254>>(n + 1, "../srs_db/ignition");
