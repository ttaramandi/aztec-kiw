...
template <typename B> inline void write(B& buf, proving_key const& key)
{
    ...
    for (size_t i = 0; i < num_polys; ++i) {
        ...
        const polynomial& value = ((proving_key&)key).polynomial_store.get(poly_id);
        write(buf, poly_id);
        write(buf, value);
    }
...
template <typename B> inline void write_to_file(B& os, std::string const& path, proving_key& key)
{
    ...
    for (size_t i = 0; i < num_polys; ++i) {
        ...
        ofs.write((char*)value.data().get(), (std::streamsize)(size * sizeof(fr)));
        if (!ofs.good()) {
