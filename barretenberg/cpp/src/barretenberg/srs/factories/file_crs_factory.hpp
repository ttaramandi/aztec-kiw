...
template <typename Curve> class FileCrsFactory : public CrsFactory<Curve> {
  public:
    ...
    std::shared_ptr<srs::factories::ProverCrs<Curve>> get_prover_crs(size_t degree) override;

    std::shared_ptr<srs::factories::VerifierCrs<Curve>> get_verifier_crs(size_t degree = 0) override;

  private:
    std::string path_;
    size_t degree_;
    std::shared_ptr<srs::factories::ProverCrs<Curve>> prover_crs_;
    std::shared_ptr<srs::factories::VerifierCrs<Curve>> verifier_crs_;
};
