...
class MemGrumpkinCrsFactory : public CrsFactory<curve::Grumpkin> {
  public:
    ...
    std::shared_ptr<bb::srs::ProverCrs<curve::Grumpkin>> get_prover_crs(size_t degree) override;

    std::shared_ptr<bb::srs::VerifierCrs<curve::Grumpkin>> get_verifier_crs(size_t degree = 0) override;

  private:
    std::shared_ptr<bb::srs::ProverCrs<curve::Grumpkin>> prover_crs_;
    std::shared_ptr<bb::srs::VerifierCrs<curve::Grumpkin>> verifier_crs_;
};
