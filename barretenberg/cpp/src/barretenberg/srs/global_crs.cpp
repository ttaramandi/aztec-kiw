...
std::shared_ptr<bb::srs::CrsFactory<bb::curve::BN254>> crs_factory;
std::shared_ptr<bb::srs::CrsFactory<bb::curve::Grumpkin>> grumpkin_crs_factory;
} // namespace
...
{
    crs_factory = std::make_shared<MemBn254CrsFactory>(points, g2_point);
}

// Initializes crs from a file path this we use in the entire codebase
void init_crs_factory(std::string crs_path)
{
    crs_factory = std::make_shared<FileCrsFactory<curve::BN254>>(crs_path);
}

// Initializes the crs using the memory buffers
void init_grumpkin_crs_factory(std::vector<curve::Grumpkin::AffineElement> const& points)
{
    grumpkin_crs_factory = std::make_shared<MemGrumpkinCrsFactory>(points);
}

void init_grumpkin_crs_factory(std::string crs_path)
{
    grumpkin_crs_factory = std::make_shared<FileCrsFactory<curve::Grumpkin>>(crs_path);
}

std::shared_ptr<CrsFactory<curve::BN254>> get_crs_factory()
{
    if (!crs_factory) {
...
std::shared_ptr<CrsFactory<curve::Grumpkin>> get_grumpkin_crs_factory()
{
