...
TYPED_TEST(ScalarMultiplicationTests, ReduceBucketsSimple)
{
    ...
    auto crs = srs::FileProverCrs<Curve>(num_points / 2, TestFixture::SRS_PATH);
    auto monomials = crs.get_monomial_points();

    std::vector<uint64_t> point_schedule(bb::point_table_size(num_points / 2));
    ...
    bb::affine_product_runtime_state<Curve> product_state{
    ...
    AffineElement* output = bb::reduce_buckets<Curve>(product_state, true);
...
TYPED_TEST(ScalarMultiplicationTests, ReduceBuckets)
{
    ...
    bb::generate_pippenger_point_table<Curve>(monomials, monomials, num_initial_points);
    ...
    bb::pippenger_runtime_state<Curve> state(num_initial_points);

    std::chrono::steady_clock::time_point start = std::chrono::steady_clock::now();
    bb::compute_wnaf_states<Curve>(
    ...
    bb::organize_buckets(state.point_schedule, num_points);
    end = std::chrono::steady_clock::now();
    diff = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
    std::cout << "organize bucket time: " << diff.count() << "ms" << std::endl;
    const size_t max_num_buckets = bb::get_num_buckets(num_points * 2);
    ...
    bb::affine_product_runtime_state<Curve> product_state{ monomials,
                                                           point_pairs,
                                                           scratch_points,
                                                           scratch_field,
                                                           bucket_counts,
    ...
    start = std::chrono::steady_clock::now();
    // bb::scalar_multiplication_internal<Curve><num_points>(state, monomials);
    end = std::chrono::steady_clock::now();
    diff = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
    ...
    AffineElement* result_buckets = bb::reduce_buckets<Curve>(product_state, true);
...
TYPED_TEST(ScalarMultiplicationTests, DISABLED_ReduceBucketsBasic)
{
    ...
    start = std::chrono::steady_clock::now();
    bb::reduce_buckets<Curve>(product_state, true);
    // bb::scalar_multiplication_internal<Curve><num_points>(state, monomials);
    end = std::chrono::steady_clock::now();
    diff = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
    std::cout << "scalar mul: " << diff.count() << "ms" << std::endl;
...
TYPED_TEST(ScalarMultiplicationTests, AddAffinePoints)
{
    ...
    bb::add_affine_points<Curve>(points, num_points, scratch_space);
...
TYPED_TEST(ScalarMultiplicationTests, ConstructAdditionChains)
{
    ...
    start = std::chrono::steady_clock::now();
    bb::construct_addition_chains<Curve>(product_state, true);
    end = std::chrono::steady_clock::now();
    diff = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
...
TYPED_TEST(ScalarMultiplicationTests, RadixSort)
{
    ...
    constexpr size_t num_rounds = bb::get_num_rounds(target_degree * 2);
    ...
    bb::pippenger_runtime_state<Curve> state(target_degree);
    bb::compute_wnaf_states<Curve>(state.point_schedule, state.skew_table, state.round_counts, scalars, target_degree);
    ...
    bb::organize_buckets(state.point_schedule, target_degree * 2);
...
TYPED_TEST(ScalarMultiplicationTests, OversizedInputs)
{
    ...
    bb::generate_pippenger_point_table<Curve>(monomials, monomials, target_degree);
    ...
    Element first = bb::pippenger<Curve>(scalars, monomials, target_degree, state);
    first = first.normalize();

    for (size_t i = 0; i < target_degree; ++i) {
        scalars[i].self_neg();
    }
    bb::pippenger_runtime_state<Curve> state_2(target_degree);

    Element second = bb::pippenger<Curve>(scalars, monomials, target_degree, state_2);
...
TYPED_TEST(ScalarMultiplicationTests, UndersizedInputs)
{
    ...
    bb::generate_pippenger_point_table<Curve>(points, points, num_points);

    bb::pippenger_runtime_state<Curve> state(num_points);

    Element result = bb::pippenger<Curve>(scalars, points, num_points, state);
...
TYPED_TEST(ScalarMultiplicationTests, PippengerShortInputs)
{
    ...
    auto points = bb::point_table_alloc<AffineElement>(num_points);
    ...
    expected = expected.normalize();
    bb::generate_pippenger_point_table<Curve>(points.get(), points.get(), num_points);
    bb::pippenger_runtime_state<Curve> state(num_points);

    Element result = bb::pippenger<Curve>(scalars, points.get(), num_points, state);
    result = result.normalize();
...
TYPED_TEST(ScalarMultiplicationTests, PippengerUnsafe)
{
    ...
    bb::pippenger_runtime_state<Curve> state(num_points);
    Element result = bb::pippenger_unsafe<Curve>(scalars, points.get(), num_points, state);
    result = result.normalize();
...
TYPED_TEST(ScalarMultiplicationTests, PippengerUnsafeShortInputs)
{
    ...
    Element result = bb::pippenger_unsafe<Curve>(scalars, points, num_points, state);
    result = result.normalize();
...
TYPED_TEST(ScalarMultiplicationTests, PippengerZeroPoints)
{
    ...
    bb::pippenger_runtime_state<Curve> state(0);
    Element result = bb::pippenger<Curve>(scalars, points, 0, state);
...
TYPED_TEST(ScalarMultiplicationTests, PippengerMulByZero)
{
    ...
    bb::generate_pippenger_point_table<Curve>(points, points, 1);

    bb::pippenger_runtime_state<Curve> state(1);
    Element result = bb::pippenger<Curve>(scalars, points, 1, state);
