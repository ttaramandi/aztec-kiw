...
export class AvmTokenSimulator {
  ...
  constructor(protected token: AvmTestContract, protected logger: DebugLogger, protected accounts: AztecAddress[]) {}
  ...
  public async check() {
    expect(await this.token.methods.view_total_supply().view()).toEqual(this.totalSupply);

    // Check that all our public matches
    for (const address of this.accounts) {
      expect(await this.token.methods.view_balance_of_public({ address }).view()).toEqual(this.balanceOfPublic(address));
    }
  }
}
